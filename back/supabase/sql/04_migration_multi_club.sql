...
--
-- Chaque CREATE POLICY est precede de deux DROP : l'ancien nom, que cette
-- migration remplace, et le nouveau. Le second ne sert qu'au rejeu sur une
-- base neuve. Le fichier 03 a ete remis a jour depuis et pose deja le modele
-- cloisonne par club, donc les trente-trois policies de ce fichier existent
-- deja quand on le lance dans l'ordre. Sans ce DROP, trente-et-une erreurs
-- sortent, toutes sans consequence, et elles masqueraient la premiere vraie.
--
-- Verifie sur PostgreSQL 16 : la sequence complete rejouee deux fois, une fois
-- en laissant ce fichier echouer, une fois en le laissant gagner, donne les
-- deux fois exactement les memes 238 policies. Les fichiers 23, 24 et 29
-- reecrivent de toute facon tout ce que ce fichier pose.
...
DROP POLICY IF EXISTS "Admins can insert club events" ON public.events;
...
DROP POLICY IF EXISTS "Admins can update club events" ON public.events;
...
DROP POLICY IF EXISTS "Admins can delete club events" ON public.events;
...
DROP POLICY IF EXISTS "Users can see club event registrations" ON public.event_players;
...
DROP POLICY IF EXISTS "Users can register to club events" ON public.event_players;
...
DROP POLICY IF EXISTS "Users can unregister from club events" ON public.event_players;
...
DROP POLICY IF EXISTS "Users can select club schedules" ON public.schedule;
...
DROP POLICY IF EXISTS "Admins can insert club schedules" ON public.schedule;
...
DROP POLICY IF EXISTS "Admins can update club schedules" ON public.schedule;
...
DROP POLICY IF EXISTS "Users can select club absences" ON public.absences;
...
DROP POLICY IF EXISTS "Users and admins can insert club absences" ON public.absences;
...
DROP POLICY IF EXISTS "Users and admins can delete club absences" ON public.absences;
...
DROP POLICY IF EXISTS "Users can view club player statuses" ON public.player_status;
...
DROP POLICY IF EXISTS "Admins can insert club player statuses" ON public.player_status;
...
DROP POLICY IF EXISTS "Admins can update club player statuses" ON public.player_status;
...
DROP POLICY IF EXISTS "Admins can delete club player statuses" ON public.player_status;
