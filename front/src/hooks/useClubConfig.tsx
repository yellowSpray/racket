    ...
    const fetchClubConfig = useCallback(async (clubId: string | null) => {
        ...
        try {
            ...
            const [clubRes, scoringRes, promotionRes] = await Promise.all([
                supabase
                    .from("clubs")
                    .select("id, club_name, club_address, club_email, logo_url, default_min_players_per_group, default_max_players_per_group, visitor_fee, open_to_visitors, auto_renew_players, default_start_time, default_end_time, default_number_of_courts, default_match_duration")
                    .eq("id", clubId)
                    .single(),
                supabase
                    .from("scoring_rules")
